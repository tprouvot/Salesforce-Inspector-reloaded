...
1. Set up an org (e.g. a Developer Edition) and apply the following customizations:
   1. Everything described in metadata in `test/`. Push to org with `sf deploy metadata -d test/ -o [your-test-org-alias]` (legacy: `sfdx force:source:deploy -p test/ -u [your-test-org-alias]`).
