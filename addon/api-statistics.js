...
export class ApiStatistics {
   ...
   */
  loadStats() {
    const stored = localStorage.getItem("apiStatistics");
    if (stored) {
      try {
  ...
  saveStats(stats = null) {
    try {
      ...
      localStorage.setItem("apiDebugStatistics", JSON.stringify(statsToSave));
  ...
  */
  trackApiCall(mode, url, method, duration, isError = false){
    ...
    const stored = localStorage.getItem("apiDebugStatistics");
    let stats;
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
  ...
  setStatsToLocalStorage(stats) {
    localStorage.setItem("apiDebugStatistics", JSON.stringify(stats));
  }
