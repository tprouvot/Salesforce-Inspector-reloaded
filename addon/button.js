...
function initButton(sfHost, inInspector) {
  ...
  function setFavicon(sfHost) {
    ...
    let {[sfHost + "_customFavicon"]: fav, [sfHost + "_isSandbox"]: isSandbox} = iFrameLocalStorage;
    if (fav) {
      ...
      if (iFrameLocalStorage.colorizeSandboxBanner && fav.indexOf("http") == -1){
        if (isSandbox){
          setTimeout(colorizeBanner, 600, fav);
        }
  ...
  function colorizeBanner(faviconColor){
    let headerSandbox = document.querySelector("#oneHeader > div.slds-color__background_gray-1.slds-text-align_center.slds-size_full.slds-text-body_regular.oneSystemMessage > div.slds-notify_alert.system-message.level-info.slds-theme_info");
    if (headerSandbox){
      headerSandbox.style.backgroundColor = faviconColor;
    }
  }
