...
function initButton(sfHost, inInspector) {
  ...
  // Initialize from localStorage if running in extension context
  if (["chrome-extension", "moz-extension"].some(p => location.protocol.startsWith(p))) {
    iFrameLocalStorage = {...localStorage};
  }
  ...
  const SELECTORS = {
    ...
    DEVOPS_CENTER: "devops_center-base-component",
    DEVOPS_NAVBAR: "navBar-container",
    GLOBAL_HEADER: "div.slds-global-header",
    ONE_HEADER: "header.oneGlobalNav",
    SYSTEM_MESSAGE: "div.slds-color__background_gray-1.slds-text-align_center.slds-size_full.slds-text-body_regular.oneSystemMessage",
    PROD_BANNER: ".sfir-prod-banner",
    BORDER_STYLE: "sfir-border-style"
  };
  ...
  function applyHeaderStyle(el, color, text) {
    ...
    if (!badge) {
      ...
      badge.className = SANDBOX_TEXT_BADGE_CLASS;
      badge.style.cssText = "position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:white;font-weight:bold;font-size:20px;z-index:9999;pointer-events:none;text-shadow:1px 1px 2px rgba(0,0,0,0.5);width:100%;text-align:center;";
  ...
  function colorizeSandboxNavbar(color, text) {
    observeElement(SELECTORS.DEVOPS_CENTER, (banner) => {
      const navBar = banner.getElementsByClassName(SELECTORS.DEVOPS_NAVBAR)[0];
      if (navBar) applyHeaderStyle(navBar, color, text);
    });
    observeElement(SELECTORS.GLOBAL_HEADER, () => {
      const header = document.querySelector(SELECTORS.ONE_HEADER);
      if (header) applyHeaderStyle(header, color, text);
    });
  }

  function resetHeaderStyle(el) {
    if (!el) return;
    el.style.backgroundColor = "";
    const badge = el.querySelector("." + SANDBOX_TEXT_BADGE_CLASS);
    if (badge) badge.remove();
  }

  function resetSandboxNavbar() {
    const banner = document.querySelector(SELECTORS.DEVOPS_CENTER);
    if (banner) resetHeaderStyle(banner.getElementsByClassName(SELECTORS.DEVOPS_NAVBAR)[0]);
    resetHeaderStyle(document.querySelector(SELECTORS.ONE_HEADER));
  }

  function addProdWarningBanner(faviconColor, bannerText) {
    const text = bannerText || "WARNING: THIS IS PRODUCTION";
    const existingBanner = document.querySelector(SELECTORS.PROD_BANNER);

    if (existingBanner) {
      existingBanner.style.backgroundColor = faviconColor;
      const span = existingBanner.querySelector("span");
      if (span) span.textContent = text;
      return;
    }
    const prodBanner = document.createElement("div");
    prodBanner.className = "slds-notify_alert sfir-prod-banner";
    prodBanner.style.backgroundColor = faviconColor;
    prodBanner.style.color = "white";

    const span = document.createElement("span");
    span.textContent = text;
    prodBanner.appendChild(span);
    const bannerContainer = document.querySelector(SELECTORS.SYSTEM_MESSAGE);
    if (bannerContainer) {
      bannerContainer.appendChild(prodBanner);
    } else {
      observeElement(SELECTORS.SYSTEM_MESSAGE, (container) => {
        if (!document.querySelector(SELECTORS.PROD_BANNER)) container.appendChild(prodBanner);
      });
    }
  }

  function removeProdWarningBanner() {
    const banner = document.querySelector(SELECTORS.PROD_BANNER);
    if (banner) banner.remove();
  }

  function addBorder(color) {
    let style = document.getElementById(SELECTORS.BORDER_STYLE);
    if (!style) {
      style = document.createElement("style");
      style.id = SELECTORS.BORDER_STYLE;
      document.head.appendChild(style);
    }
    style.textContent = `body::before { content: ''; display: block; position: fixed; width: 100%; height: 2px; background-color: ${color}; z-index: 9999; }`;
  }

  function removeBorder() {
    const style = document.getElementById(SELECTORS.BORDER_STYLE);
    if (style) style.remove();
  }

  function hostsMatch(h1, h2) { return h1 && h2 && h1.toLowerCase() === h2.toLowerCase(); }
  ...
  function loadPopup(sfHost) {
    ...
    function handleSettingsUpdate(data) {
      const {settings, sfHost: messageSfHost} = data;
      if (!settings) {
        return;
      }

      const isTargetHost = hostsMatch(messageSfHost, sfHost);

      if (isTargetHost) {
        if (settings.customFavicon !== undefined) iFrameLocalStorage[`${sfHost}_customFavicon`] = settings.customFavicon;
        if (settings.orgBannerText !== undefined) iFrameLocalStorage[`${sfHost}_orgBannerText`] = settings.orgBannerText;
      }

      ["overrideColorsOption", "colorizeFavicon", "colorizeOrgBanner", "colorizeExtHeader", "popupArrowOrientation", "popupArrowPosition"]
        .forEach(key => {
          if (settings[key] !== undefined) {
            iFrameLocalStorage[key] = settings[key];
          }
        });

      applyAppearanceForHost(sfHost);
      if (settings.popupArrowOrientation !== undefined || settings.popupArrowPosition !== undefined) {
        updatePopupPosition();
      }
    }
    ...
    function updatePopupPosition() {
      ...
      const dir = calcDirection(iFrameLocalStorage.popupArrowPosition, o);
      resetPopupClass(o);
      if (dir) popupEl.classList.add(`insext-popup-${o}-${dir}`);
      const oldBtnClass = btn.className.split(" ").find(c => c.startsWith("insext-btn-"));
      if (oldBtnClass) btn.classList.remove(oldBtnClass);
      while (btn.firstChild) btn.removeChild(btn.firstChild);
      ["top", "right", "bottom", "left"].forEach(prop => { rootEl.style[prop] = ""; });
    ...
    addEventListener("message", e => {
      // Accept messages from the popup iframe (normal flow) and from the parent
      // window (used by options page for local dynamic updates)
      ...
      if (e.data.insextSettingsUpdate) {
        handleSettingsUpdate(e.data);
      }
      if (e.data.insextOrgInfoUpdate) {
        if (e.data.sfHost === sfHost) {
          iFrameLocalStorage[`${sfHost}_isSandbox`] = String(e.data.isSandbox);
          iFrameLocalStorage[`${sfHost}_trialExpirationDate`] = e.data.trialExpirationDate;
          applyAppearanceForHost(sfHost);
        }
      }
