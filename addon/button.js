...
function initButton(sfHost, inInspector) {
  ...
  function setRootCSSProperties(rootElement, buttonElement) {
    ...
    // Clean previous orientation class and arrow image to avoid duplicates
    buttonElement.classList.remove("insext-btn-vertical", "insext-btn-horizontal");
    const existingArrow = buttonElement.querySelector('img[role="presentation"]');
    if (existingArrow) {
      buttonElement.removeChild(existingArrow);
    }
    // Reset any previously set positioning styles to avoid conflicts when switching orientation
    ["top", "right", "bottom", "left"].forEach(side => { rootElement.style[side] = ""; });
  ...
  function updateBanner(banner, faviconColor){
    ...
    // Apply white text with !important to override SLDS themes
    [...banner.children].forEach(child => child.style.setProperty("color", "white", "important"));
    if (newBanner){
  ...
  function loadPopup(sfHost) {
    ...
    addEventListener("message", e => {
      ...
      // React to option changes made in the Options page and broadcasted via popup iframe
      if (e.data.updateLocalStorage) {
        // Update our in-memory copy of filtered storage so subsequent logic sees new values
        iFrameLocalStorage[e.data.key] = e.data.value;
        // Request the popup to refresh and resend the filtered storage so we stay in sync
        popupEl.contentWindow.postMessage({refreshFilteredStorage: true}, "*");
        // Re-apply favicon/banner with latest settings immediately
        setFavicon(sfHost);
        // If arrow orientation/position changed, re-apply popup/button placement immediately
        if (e.data.key === "popupArrowPosition" || e.data.key === "popupArrowOrientation") {
          setRootCSSProperties(rootEl, btn);
          const o = getOrientation("iframe");
