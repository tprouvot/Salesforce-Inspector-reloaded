...
export async function dataImportTest(test) {
  ...
  records = getRecords(await sfConn.rest("/services/data/" + apiVersion + "/query/?q=" + encodeURIComponent("select Name, Checkbox__c, Number__c, Lookup__r.Name from Inspector_Test__c order by Name")));
  assertEquals([
  ...
  // Update csv
  records = getRecords(await sfConn.rest("/services/data/" + apiVersion + "/query/?q=" + encodeURIComponent("select Id from Inspector_Test__c order by Name")));
  vm.dataFormat = "csv";
  vm.didUpdate();
  vm.importAction = "update";
  vm.didUpdate();
  ...
  records = getRecords(await sfConn.rest("/services/data/" + apiVersion + "/query/?q=" + encodeURIComponent("select Name from Inspector_Test__c order by Name")));
  assertEquals([
