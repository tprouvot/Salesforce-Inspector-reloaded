...
| Over-customization | On / warning | Too many custom fields (100/300), relationships (25/40), record types (50), or custom objects in the scan set (100/200) |
...
2. Search filters the loaded schema. `Object.Field` (for example `Account.Name`) matches that object token, then fields whose API name, label, description, or classification contains the second part. A value without a dot matches objects or fields.
...
**XLS** copies the loaded objects and fields to the clipboard (tab-separated, for Excel). **CSV** downloads the same data as a file. Description and classification columns are included only when FieldDefinition was queried.
