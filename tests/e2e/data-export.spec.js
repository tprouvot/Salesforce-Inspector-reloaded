...
test.describe("Data Export", () => {
  ...
  test("Autocomplete Suggestions", async ({page, context, extensionId}) => {
    ...
    // Position cursor after "Nam" so autocomplete suggests Account fields
    await queryInput.fill("SELECT Id, Nam FROM Account");
    // Move cursor from end (29) to position 14 (after "Nam") - keyup triggers autocomplete
    for (let i = 0; i < 15; i++) {
      await queryInput.press("ArrowLeft");
    }

    // Wait for autocomplete debounce (100ms) and field suggestions
