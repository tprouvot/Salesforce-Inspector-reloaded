...
test.describe("Options", () => {
  ...
  test("Load Options Page", async ({page, extensionId}) => {
    ...
    await page.waitForSelector("h1:has-text('Options'), .slds-text-heading_small:has-text('Options')", {timeout: 15000});
  ...
  test("Add Custom Shortcut", async ({page, extensionId}) => {
    ...
    await page.waitForSelector("table.slds-table tbody tr input[type='text']", {timeout: 5000});
  ...
  test("Import Options", async ({page, extensionId}) => {
    ...
    await page.waitForSelector(".slds-notify", {timeout: 5000});

    // Verify success toast appears
    await expect(page.locator("text=Options Imported Successfully!")).toBeVisible({timeout: 5000});
  });
